/* ══════════════════════════════════════════
   ...
   Ahora: fetch() contra la API en PHP + MySQL (XAMPP)
══════════════════════════════════════════ */
 ...
 * Además, compara contra la versión anterior para detectar cambios de
 * estado (la API los avanza sola con el tiempo) y avisar al usuario.
 */
async function loadUserRepairs() {
  if (!Store.getToken()) { state.repairs = []; renderNotifBadge(); return; }

  const previous = state.repairs;
  try {
    const fresh = await apiFetch('/repairs_list.php');
    notifyRepairStatusChanges(previous, fresh);
    state.repairs = fresh;
  } catch (err) {
    state.repairs = [];
    showToast(err.message);
...
const STATUS_CHANGE_MESSAGES = {
  active:    (r) => `🚚 ¡Tu técnico va en camino! ${r.appliance} · ${r.code}`,
  completed: (r) => `✅ Reparación completada: ${r.appliance} · ${r.code}. ¡Califica el servicio!`,
};

/**
 * Compara el historial anterior contra el nuevo y muestra un aviso
 * (toast + notificación del navegador) por cada reparación que cambió
 * de estado desde la última vez que se consultó.
 */
function notifyRepairStatusChanges(previousRepairs, freshRepairs) {
  if (!previousRepairs || previousRepairs.length === 0) return; // primera carga: no avisar de todo

  const prevById = Object.fromEntries(previousRepairs.map(r => [r.id, r.status]));

  freshRepairs.forEach(r => {
    const before = prevById[r.id];
    if (before && before !== r.status && STATUS_CHANGE_MESSAGES[r.status]) {
      const msg = STATUS_CHANGE_MESSAGES[r.status](r);
      showToast(msg);
      sendBrowserNotification('Error+', msg);
    }
  });
}
...
function sendBrowserNotification(title, body) {
  ...
  if (Notification.permission === 'granted') {
    new Notification(title, { body, icon: 'https://em-content.zobj.net/thumbs/120/apple/354/wrench_1f527.png' });
  }
}

let repairsPollTimer = null;

/** Revisa el historial cada 20s mientras haya sesión activa, para que
 *  los cambios de estado (simulados por la API) lleguen solos, sin
 *  que el usuario tenga que recargar la página. */
function startRepairsPolling() {
  stopRepairsPolling();
  repairsPollTimer = setInterval(() => {
    if (Store.getToken()) loadUserRepairs();
  }, 20000);
}

function stopRepairsPolling() {
  if (repairsPollTimer) clearInterval(repairsPollTimer);
  repairsPollTimer = null;
}
...
function buildRepairCard(r) {
  ...
  let actionBtn = '';
  if (r.status === 'pending') {
    actionBtn = `<button class="rate-btn" onclick="advanceRepairStatus('${r.id}')"><span class="material-symbols-outlined" style="font-size:16px;">local_shipping</span>Marcar en curso</button>`;
  } else if (r.status === 'active') {
    actionBtn = `<button class="rate-btn" onclick="advanceRepairStatus('${r.id}')"><span class="material-symbols-outlined" style="font-size:16px;">task_alt</span>Marcar completado</button>`;
  } else if (r.status === 'completed' && !rated) {
    actionBtn = `<button class="rate-btn" onclick="openRateModal('${r.id}')"><span class="material-symbols-outlined" style="font-size:16px;">star</span>Calificar</button>`;
  } else if (rated) {
    actionBtn = `<span style="font-size:12px; color:var(--outline); display:flex; align-items:center; gap:4px;"><span class="material-symbols-outlined" style="font-size:15px; color:var(--secondary-container); font-variation-settings:'FILL' 1,'wght' 400,'GRAD' 0,'opsz' 24;">star</span>${r.rating}.0 calificado</span>`;
  }
...
/**
 * Avanza el estado de una reparación: pendiente → en curso → completado.
 * En una app real esto lo haría el técnico desde su propia app; aquí lo
 * simula el usuario para poder probar todo el flujo sin editar phpMyAdmin.
 */
async function advanceRepairStatus(repairId) {
  try {
    const result = await apiFetch(`/repairs_status.php?id=${repairId}`, { method: 'PUT', body: '{}' });
    const labels = { active: 'en curso', completed: 'completada' };
    showToast(`Reparación marcada como ${labels[result.status] || result.status} ✅`);
    await loadUserRepairs();
    renderRepairList();
    renderHistoryStats();
  } catch (err) {
...
async function submitRating() {
  ...
  try {
    const result = await apiFetch(`/repairs_rate.php?id=${state.ratingRepairId}`, {
      method: 'POST',
      body: JSON.stringify({ stars: state.ratingVal, comment: document.getElementById('rate-comment').value.trim() })
    });
    closeRateModal();
    showToast(`¡Gracias por tu calificación! ⭐ Nuevo rating del técnico: ${result.technicianNewRating}`);
    await loadUserRepairs();
    renderRepairList();
    renderHistoryStats();
  } catch (err) {
...
function renderProfileLoggedIn() {
  ...
  pv.innerHTML = `
    ...
    <div class="profile-section">
      ...
      <div class="profile-row" onclick="openEditProfileModal()">
...
/* ══════════════════════════════════════════
   ...
   de reparaciones del usuario, vía API)
══════════════════════════════════════════ */
...
/* ══════════════════════════════════════════
   EDITAR PERFIL (PUT /user_update.php)
══════════════════════════════════════════ */
function openEditProfileModal() {
  const u = state.currentUser;
  if (!u) return;
  if (!u.id) {
    showToast('Crea una cuenta para poder editar tu información.');
    return;
  }
...
async function saveProfileEdits() {
  ...
  try {
    const updated = await apiFetch('/user_update.php', {
      method: 'PUT',
...
// Aplica el tema guardado (o el que prefiera el sistema operativo) al cargar
(function initTheme() {
